...
exports.createCalendarEvent = async (req, res) => {
  ...
  const newEvent = await new EventModel({
    ...
    status: body.status,
    color: body.color,
    stdEventStart: body.stdEventStart,
    stdEventEnd: body.stdEventEnd,
  }).save();
  let calendarUser = await CalendarModel.findOne({ id: body.id });
  if (!calendarUser) await new CalendarModel({ id: body.id }).save();
...
exports.getCalendarEvent = async (req, res) => {
  ...
  if (!calendarUser) {
    ...
    await res.send({
      events: await calendarUser.populate("events"),
      msg: "get Event",
    });
  }
};
